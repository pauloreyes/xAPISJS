...
document.addEventListener('visibilitychange', function () {
  if (document.visibilityState === 'visible') {
    ...
    if (windowInactiveState === true && jsname != null && jsemail != null) {
      ...
      sendXAPI(
        ...
        "The e-learning window was placed on the background. It's possible that the learner did something else.",
        jsemail,
        jsname,
        'inactivity',
        false,
...
let cdTime; //determines how long the countdown will be. This is called in the module.
...
let leaderboardUpdate = () => {
  ...
  params['activity'] = 'http://slide1.7';
  xAPIQuery = ADL.XAPIWrapper.getStatements(params);
  ...
  player.SetVar('ThirdPlace', statementsHolder[2]?.actor?.name ?? 'No entry');
  player.SetVar('FourthPlace', statementsHolder[3]?.actor?.name ?? 'No entry');
  player.SetVar('FifthPlace', statementsHolder[4]?.actor?.name ?? 'No entry');
  player.SetVar('SixthPlace', statementsHolder[5]?.actor?.name ?? 'No entry');
  player.SetVar('SeventhPlace', statementsHolder[6]?.actor?.name ?? 'No entry');
  player.SetVar('EighthPlace', statementsHolder[7]?.actor?.name ?? 'No entry');
  player.SetVar('NinthPlace', statementsHolder[8]?.actor?.name ?? 'No entry');
  player.SetVar('TenthPlace', statementsHolder[9]?.actor?.name ?? 'No entry');
};
